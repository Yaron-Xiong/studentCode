 ...
 * <p>
 * 来源：力扣（LeetCode）
 * 链接：<a href="https://leetcode.cn/problems/earliest-finish-time-for-land-and-water-rides-ii/">...</a>
 * 著作权归领扣网络所有。商业转载请联系官方授权，非商业转载请注明出处。
 */
public class L3635_EarliestFinishTime {
    public int earliestFinishTime(int[] landStartTime, int[] landDuration, int[] waterStartTime, int[] waterDuration) {
        int ans1 = solute(landStartTime, landDuration, waterStartTime, waterDuration);
        int ans2 = solute(waterStartTime, waterDuration, landStartTime, landDuration);
        return Math.min(ans1, ans2);
    }

    private int solute(int[] landStartTime, int[] landDuration, int[] waterStartTime, int[] waterDuration) {
        //先求出其中一个的最小完成时间
        ...
        for (int i = 0; i < waterDuration.length; i++) {
            ans = Math.min(ans, Math.max(minFinish, waterStartTime[i]) + waterDuration[i]);
        }
        return ans;
    }
}
